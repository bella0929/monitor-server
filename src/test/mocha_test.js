...
const assert = require('assert'); // redis服务器地址
...
describe('Mysql 数据库环境断言', () => {
  it(mysql_host + ':' + mysql_port, done => {
    ...
    const Knex = knex({
      ...
      log: {
        error(message) {// assert.equal(1 > 0, true)
        }

      }
    });
    Knex.select('*').from('t_o_project').then(function (response) {
...
describe('Redis 缓存环境断言', () => {
  it(redis_host + ':' + redis_port, done => {
    var redisClient = new Redis({
      ...
      lazyConnect: true,
      // 初始化时不能连接Redis Server, 否则会因为无法断开连接, 导致npm run fee命令不能退出
...
//# sourceMappingURL=mocha_test.js.map