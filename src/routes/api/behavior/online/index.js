...
async function online(req, res) {
  ...
  let projectId = _.get(req, ['fee', 'project', 'projectId'], 0); // 传上来的时间戳是毫秒级的
  ...
  let orderMap = new Map(); // 对于按周期查看的情况, 需要保证在周期内有所有的数据
  ...
  for (let checkAtMoment = startAtMoment.clone(); checkAtMoment.isBefore(endAtMoment); checkAtMoment = checkAtMoment.clone().add(1, momentIncreaceStep)) {
    ...
    switch (filterBy) {
      ...
      case REQUEST_FILTER_BY_DAY:
        format = 'YYYY/MM/DD';
        startAtKey = checkAtMoment.clone().format(format); // endAtKey = checkAtMoment.clone().subtract(1, 'days').format(format)
  ...
  for (let rawRecord of rawRecordList) {
    ...
    switch (filterBy) {
      ...
      case REQUEST_FILTER_BY_DAY:
        format = 'YYYY/MM/DD';
        startAtKey = countAtMoment.clone().format(format); // endAtKey = countAtMoment.clone().subtract(1, 'days').format(format)
...
let onlineRouterConfig = RouterConfigBuilder.routerConfigBuilder('/api/behavior/online', RouterConfigBuilder.METHOD_TYPE_GET, online);
export default { ...onlineRouterConfig
};
