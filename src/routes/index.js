...
const baseRouter = express.Router(); // 路由分为以下部分
// 不需要登录

const withoutLoginRouter = express.Router(); // 需要登录

const loginRouter = express.Router(); // 需要登录-需要项目id

const loginProjectRouter = express.Router(); // 需要登录-不需要项目id

const loginCommonRouter = express.Router();
let routerConfigMap = { ...Api
};
/**
 * 根据请求方法注册路由
...
loginProjectRouter.use(PrivilegeChecker.checkPrivilege); // 自动注册
...
for (let url of Object.keys(routerConfigMap)) {
  ...
  if (routerConfig.needLogin) {
    ...
    Logger.log(`不需要登录(Method: ${routerConfig.methodType}) =>`, url); // 不需要登录
