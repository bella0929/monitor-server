...
const TABLE_COLUMN = [`id`, `project_id`, `total_stay_ms`, `total_uv`, `count_at_time`, `count_type`, `city_distribute_id`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function getRecordList(projectId, startAt, finishAt, countType) {
  ...
  let recordList = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', projectId).andWhere('count_at_time', 'in', countAtTimeList).andWhere('count_type', '=', countType).orderBy('count_at_time', 'asc').catch(e => {
    return [];
  });
  return recordList;
}
...
//# sourceMappingURL=duration_distribution.js.map