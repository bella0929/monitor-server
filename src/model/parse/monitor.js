...
const TABLE_COLUMN = [`id`, `error_type`, `error_name`, `http_code`, `monitor_ext_id`, `during_ms`, `request_size_b`, `response_size_b`, `url`, `country`, `province`, `city`, `log_at`, `md5`, `create_time`, `update_time`];
/**
 * @param {*} projectId
 * @param {*} visitAt
 */
...
async function groupBy(params) {
  ...
  let res = await Knex(table).select(column).count('* as error_count').groupBy(column).where(whereParams).andWhere('log_at', '>', startAt).andWhere('log_at', '<', endAt).orderBy('error_count', 'desc').limit(MAX_DISPLAY_ERROR).catch(() => {
    return [];
  });
  return res;
}
...
async function getErrorCountInRangeBySameMonth(projectId, startAt, finishAt) {
  ...
  let rawRecordList = await Knex.count('* as error_count').from(tableName).where('log_at', '>', startAt).andWhere('log_at', '<', finishAt).catch(e => {
    return [];
  });
...
async function getErrorNameDistributionInSameMonth(projectId, startAt, finishAt, errorType, url, groupBy = QUERY_GROUP_BY_MINUTE) {
  ...
  const tableName = getTableName(projectId, startAt); // 返回值demo =>
  ...
  let rawRecordList = await Knex.count('* as error_count').select('error_name', Knex.raw(`FROM_UNIXTIME(\`log_at\`, '${sqlGroupByFormat}') as group_by`)).from(tableName).where('log_at', '>', startAt).andWhere('log_at', '<', finishAt).andWhere(condition).groupBy('error_name', 'group_by').orderBy('error_count', 'desc').limit(MAX_DISPLAY_ERROR).catch(e => {
    return [];
  }); // 将group_by转换成时间戳
...
async function getErrorCountForAlarm(projectId, errName, startAt, endAt) {
  ...
  const rawRecordList = await Knex.count('id as error_count').from(tableName).where(whereParams).andWhere('log_at', '>=', startAt).andWhere('log_at', '<=', endAt).catch(err => {
...
async function getErrorNameList(projectId, errorType) {
  ...
  const rawRecordList = await Knex.select().distinct('error_name').from(function () {
    this.select(['id', 'error_name']).from(tableName).where('error_type', errorType).orderBy('id', 'desc').where('log_at', '>', sevenDayAgoAt).andWhere('log_at', '<', nowAt).limit(MAX_SEARCH_ERROR_NAME).as('t1');
  }).catch(err => {
...
async function getRecordListInRange(projectId, startAt, endAt) {
  ...
  const rawRecordList = await Knex.select(TABLE_COLUMN).from(tableName).where('log_at', '>', startAt).andWhere('log_at', '<', endAt).catch(err => {
...
async function getTotalCountByConditionInSameMonth(projectId, startAt, endAt, offset = 0, max = 10, errorNameList = [], url = '') {
  ...
  let rawRecordList = await Knex.count('* as total_count').from(tableName).where('log_at', '>', startAt).andWhere('log_at', '<', endAt).whereIn('error_name', errorNameList).andWhere(builder => {
...
async function getListByConditionInSameMonth(projectId, startAt, endAt, offset = 0, max = 10, errorNameList = [], url = '') {
  let tableName = getTableName(projectId, startAt); // 获取最大id

  let rawResult = await Knex.max('id as maxId').from(tableName).catch(err => {
  ...
  let rawRecordList = await Knex.select(TABLE_COLUMN).from(tableName).where('log_at', '>', startAt).andWhere('id', '>', maxId - MAX_ERROR_LOG_LENGTH).andWhere('log_at', '<', endAt).whereIn('error_name', errorNameList).andWhere(builder => {
    // 外部传入的url可能是去除get参数后的结果, 所以需要进行模糊匹配
    // @todo(yaozeyuan) 添加字段, 记录 页面真实地址, 以和url进行区分
    if (url.length > 0) {
      builder.where('url', 'like', `%${url}%`);
    }
  }).orderBy('log_at', 'desc').offset(offset).limit(max).catch(e => {
    Logger.warn(e);
    return [];
  });
  let extendLogIdList = [];
  ...
  for (let rawRecord of rawRecordList) {
    let extendRecordId = _.get(rawRecord, ['monitor_ext_id'], 0); // 所有记录一定在同一张扩展表里
