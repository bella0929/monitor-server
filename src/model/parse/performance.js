...
const INDICATOR_TYPE_页面完全加载耗时 = 'load_complete_ms'; // 只能通过中括号形式设置key值
...
INDICATOR_TYPE_MAP[INDICATOR_TYPE_页面完全加载耗时] = '页面完全加载耗时'; // 指标列表

const INDICATOR_TYPE_LIST = Object.keys(INDICATOR_TYPE_MAP);
const TABLE_COLUMN = [`id`, `sum_indicator_value`, `pv`, `indicator`, `url`, `city_distribute_id`, `count_at_time`, `count_type`, `create_time`, `update_time`];
/**
 * 获取表名
 * @param {*} projectId
...
async function get(projectId, url, indicator, countAt, countType = DATE_FORMAT.UNIT.MINUTE) {
  ...
  let rawRecord = await Knex.select(TABLE_COLUMN).from(tableName).where({
    url: url,
    indicator: indicator,
...
async function getList(projectId, startAt, finishAt, condition = {}, countType = DATE_FORMAT.UNIT.MINUTE) {
  ...
  let countAtTimeList = []; // 获取所有可能的countAtTime
  ...
  for (let tableName of tableNameList) {
    let rawRecordList = await Knex.select(TABLE_COLUMN).from(tableName).where('count_type', '=', countType).whereIn('count_at_time', countAtTimeList).andWhere(builder => {
...
async function getDistinctUrlListInRange(projectId, indicatorList, startAt, endAt, countType = DATE_FORMAT.UNIT.MINUTE) {
  ...
  for (let tableName of tableNameList) {
    ...
    let rawRecordList = await Knex.distinct(['url']).from(tableName).where({
      count_type: countType
    }).whereIn('indicator', indicatorList).whereIn('count_at_time', countAtTimeList).catch(e => {
      Logger.warn('查询失败, 错误原因 =>', e);
      return [];
    });
...
async function getUrlOverviewInSameMonth(projectId, urlList, startAt, endAt, countType) {
  ...
  console.log('tableName,countType,urlList,countAtTimeList', tableName, countType, urlList, countAtTimeList); // 查询数据库

  let rawRecordList = await Knex.select(['url', 'count_type', 'indicator']).sum('sum_indicator_value as total_sum_indicator_value').sum('pv as total_pv').from(tableName).where({
    count_type: countType
  }).whereIn('url', urlList).whereIn('count_at_time', countAtTimeList).groupBy(['url', 'count_type', 'indicator']).catch(e => {
    Logger.warn('查询失败, 错误原因 =>', e);
    return [];
  });
  let rawOverview = {};
...
async function getIndicatorLineChartDataInSameMonth(projectId, url, indicator, startAt, endAt, countType) {
  ...
  for (let countStartAtMoment = startAtMoment.clone(); countStartAtMoment.unix() < endAt; countStartAtMoment = countStartAtMoment.clone().add(1, countType)) {
    let formatCountAtTime = countStartAtMoment.format(DATE_FORMAT.DATABASE_BY_UNIT[countType]);
    countAtTimeList.push(formatCountAtTime); // 将来会以时间戳为key, 对数据进行排序

    unixKeyList.push(countStartAtMoment.unix());
  }

  console.log('tableName,countType,url,indicator,countAtTimeList', tableName, countType, url, indicator, countAtTimeList); // 查询数据库

  let rawRecordList = await Knex.select(['sum_indicator_value', 'pv', 'count_at_time']).from(tableName).where({
    count_type: countType
  }).where('url', url).where('indicator', indicator).whereIn('count_at_time', countAtTimeList).catch(e => {
    Logger.warn('查询失败, 错误原因 =>', e);
    return [];
  });
...
async function getCityDistributeInRange(projectId, urlList, indicatorList, startAt, endAt, countType = DATE_FORMAT.UNIT.MINUTE) {
  let cityDistributeTotal = {}; // uv记录表按月分表, 因此需要分月计算总uv

  let rawRecordList = await getList(projectId, startAt, endAt, {
    urlList,
    indicatorList
  }, countType);
  ...
  for (let rawRecord of rawRecordList) {
    ...
    cityDistributeTotal = MCityDistribution.mergeDistributionData(cityDistributeItem, cityDistributeTotal, (cityDataItem, cityDataTotal) => {
      let result = {};
...
async function replaceInto(projectId, url, indicator, countAt, countType = DATE_FORMAT.UNIT.MINUTE, sumIndicatorValue = 0, pv = 0, cityDistribute = {}) {
  ...
  Logger.log('performance updateAt', updateAt); // 返回值是一个列表

  let oldRecordList = await Knex.select([`id`, `create_time`, `city_distribute_id`]).from(tableName).where({
    url: url,
    indicator: indicator,
    count_at_time: countAtTime,
    count_type: countType
  ...
  Logger.log(`performance ${tableName} oldRecordList`, oldRecordList); // 利用get方法, 不存在直接返回0, 没毛病
  ...
  if (id > 0) {
    ...
    .insert(data).into(tableName).catch(e => {
