...
const TABLE_COLUMN = [`id`, `ucid`, `first_visit_at`, `country`, `province`, `city`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function getList(projectId, startAt, endAt) {
  ...
  let recordList = await Knex.select(TABLE_COLUMN).from(tableName).where('first_visit_at', '>=', startAt).andWhere('first_visit_at', '<=', endAt).catch(e => {
    return [];
  });
  return recordList;
}
...
async function replaceInto(projectId, ucid, firstVisitAt, country, province, city) {
  ...
  let oldRecordList = await Knex.select([`id`, `first_visit_at`]).from(tableName).where('ucid', '=', ucid).catch(() => {
    return [];
...
async function filterExistUcidSetInDb(projectId, allUcidList) {
  let tableName = getTableName(projectId);
  let rawRecordList = await Knex.select('ucid').from(tableName).whereIn('ucid', allUcidList);
...
//# sourceMappingURL=user_first_login_at.js.map