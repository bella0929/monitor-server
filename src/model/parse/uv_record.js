...
const TABLE_COLUMN = [`id`, `uuid`, `country`, `province`, `city`, `visit_at_hour`, `pv_count`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function replaceUvRecord(projectId, uuid, visitAt, country, province, city) {
  ...
  let oldRecordList = await Knex.select([`id`]).from(tableName).where('uuid', '=', uuid).andWhere('visit_at_hour', '=', visitAtHour).catch(() => {
    return [];
...
async function getExistUuidSetInHour(projectId, visitAt) {
  let visitAtHour = moment.unix(visitAt).format(VisitAtHourDateFormat);
  let tableName = getTableName(projectId, visitAt);
  let rawRecordList = await Knex.select('uuid').from(tableName).where('visit_at_hour', '=', visitAtHour).catch(e => {
    return [];
  });
  let uuidSet = new Set();
...
async function getCityDistributeInRange(projectId, startAt, finishAt) {
  ...
  let cityDistribute = {}; // uv记录表按月分表, 因此需要分月计算总uv
  ...
  for (let currentAtMoment = startAtMoment; currentAtMoment.isBefore(finishAtMoment); currentAtMoment = currentAtMoment.clone().add(1, 'months')) {
    ...
    let rawRecordList = await Knex.countDistinct('uuid as uv_count').select([`country`, `province`, `city`]).from(tableName).where('create_time', '>', startAt).andWhere('create_time', '<', finishAt).groupBy([`country`, `province`, `city`]).catch(() => {
      return [];
    });
...
//# sourceMappingURL=uv_record.js.map