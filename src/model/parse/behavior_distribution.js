...
const TABLE_COLUMN = [`id`, `project_id`, `code`, `name`, `url`, `total_count`, `count_at_time`, `count_type`, `city_distribute_id`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function replaceRecord(projectId, code, name, url, totalCount, countAtTime, countType, cityDistribute) {
  ...
  let oldRecordList = await Knex.select([`total_count`, `city_distribute_id`, `create_time`, `id`]).from(tableName).where('project_id', '=', projectId).andWhere('count_at_time', '=', countAtTime).andWhere('code', '=', code).andWhere('count_type', '=', countType).catch(() => {
    return [];
...
async function getRecordList(projectId, startAt, finishAt, countType) {
  ...
  let recordList = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', projectId).andWhere('count_at_time', 'in', countAtTimeList).andWhere('count_type', '=', countType).orderBy('count_at_time', 'desc').catch(() => {
    return [];
  });
  return recordList;
}
...
//# sourceMappingURL=behavior_distribution.js.map