...
const TABLE_COLUMN = [`id`, `ext_json`, `create_time`, `update_time`];
/**
 * @param {*} projectId
 * @param {*} visitAt
 */
...
async function getRecordListByIdList(projectId, createAt, idList = []) {
  ...
  let rawRecordList = Knex.select(TABLE_COLUMN).from(tableName).whereIn('id', idList).catch(e => {
...
//# sourceMappingURL=monitor_ext.js.map