...
const TABLE_COLUMN = [`id`, `city_distribute_json`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function getCityDistributionRecord(id, projectId, createTimeAt) {
  ...
  let recordList = await Knex.select(TABLE_COLUMN).from(tableName).where('id', '=', id).catch(e => {
    return [];
  });
...
async function updateCityDistributionRecord(id, projectId, createTimeAt, cityDistributeJson) {
  ...
  let affectRows = await Knex(tableName).update(data).where('id', '=', id).catch(e => {
...
function mergeDistributionData(distributionSource, distributionDist, processCityData = (cityDataDist, cityDataSource) => {
...
async function getByIdListInOneMonth(projectId, cityDistributionIdList, createTimeAt) {
  ...
  const rawRecordList = await Knex.select(TABLE_COLUMN).from(talbeName).whereIn('id', cityDistributionIdList).catch(err => {
...
//# sourceMappingURL=city_distribution.js.map