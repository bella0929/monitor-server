...
async function insertInto(infos) {
  ...
  return Knex(TableName).insert(datas).catch(() => {
    return 0;
  });
}
...
async function getRecordList(infos) {
  ...
  const TableName = getTableName(tableName, splitBy, projectId);
  return Knex(TableName).select(select).where(where).catch(() => {
    return [];
  });
}
...
async function updateInto(params) {
  ...
  datas['update_time'] = updateAt;
  const TableName = getTableName(tableName, splitBy, projectId);
  return Knex(TableName).where(where).update(datas).catch(() => {
    return 0;
  });
}
/**
...
async function replaceInto(params) {
  ...
  if (id > 0) {
    datas['update_time'] = updateAt;
    const affectRows = await Knex(table).where(`id`, '=', id).update(datas).catch(() => {
      return 0;
    });
    ...
    datas['create_time'] = updateAt;
    datas['update_time'] = updateAt;
    const insertId = await Knex.insert(datas).into(table).catch(() => {
      return 0;
    });
