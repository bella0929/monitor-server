...
const TABLE_COLUMN = [`id`, `project_id`, `device_vendor`, `device_model`, `total_count`, `count_at_month`, `city_distribute_id`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function summarySystemDevice(visitAt) {
  ...
  for (let rawProject of projectList) {
    ...
    const sumRes = await Knex.count('* as total_count').select([`device_vendor`, `device_model`, `visit_at_month`, `country`, `province`, `city`]).from(systemTableName).where('visit_at_month', '=', visitAtMonth).groupBy('device_vendor').groupBy('device_model').groupBy('country').groupBy('province').groupBy('city').catch(err => {
      Logger.error(err);
      return [];
    });
    ...
    for (let countItem of sumRes) {
      ...
      if (_.has(deviceVersionRecord, deviceVersionKey)) {
        ...
        let cityDistribute = MCityDistribution.mergeDistributionData(distribution, oldDistribution, (newCityRecord, oldCityRecord) => {
...
async function replaceAndAutoIncreaseDeviceRecord(projectId, recordInfo, cityDistribute) {
  ...
  let updateAt = moment().unix(); // 返回值是一个列表

  let oldRecordList = await Knex.select([`total_count`, `city_distribute_id`, `create_time`, `id`]).from(tableName).where('project_id', '=', projectId).andWhere('count_at_month', '=', countAtMonth).andWhere('device_vendor', '=', deviceVendor).andWhere('device_model', '=', deviceModel).catch(() => {
    return [];
  ...
  if (id > 0) {
    ...
    let insertResult = await Knex.returning('id').insert(data).into(tableName).catch(e => {
      return [];
    });
...
async function getRecord(projectId, countAtMonth) {
  let tableName = getTableName();
  let recordList = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', '=', projectId).andWhere('count_at_month', '=', countAtMonth).catch(() => {
    return [];
  });
...
//# sourceMappingURL=system_device.js.map