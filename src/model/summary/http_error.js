...
const TABLE_COLUMN = [`id`, `project_id`, `total_count`, `http_code_2xx_count`, `http_code_3xx_count`, `http_code_4xx_count`, `http_code_5xx_count`, `http_code_other_count`, `city_distribute_id`, `count_type`, `count_at_time`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function summaryHttpError(visitAt, countType, startTimestamp, endTimestamp) {
  ...
  for (let rawProject of projectList) {
    ...
    const sumRes = await Knex.count('* as total_count').select([`http_code`, `country`, `province`, `city`]).from(systemTableName).where('http_code', '>', 0).andWhere('log_at', '>', startTimestamp).andWhere('log_at', '<', endTimestamp).groupBy('http_code').groupBy('country').groupBy('province').groupBy('city').catch(err => {
      Logger.error(err);
      return [];
    });
...
async function replaceAndAutoIncreaseHttpErrorRecord(projectId, countType, countAtTime, recordInfo, cityDistribute) {
  ...
  let oldRecordList = await Knex.select([`total_count`, `city_distribute_id`, `create_time`, `id`]).from(tableName).where('project_id', '=', projectId).andWhere('count_at_time', '=', countAtTime).catch(err => {
    Logger.error(err);
    return [];
  ...
  let createTimeInDb = _.get(oldRecordList, [0, 'create_time'], 0); // let cityDistributeJsonInDb = '{}'
...
async function getRecord(projectId, countAtMonth) {
  let tableName = getTableName();
  let recordList = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', '=', projectId).andWhere('count_at_time', '=', countAtMonth).catch(err => {
    Logger.error(err);
    return [];
  });
...
//# sourceMappingURL=http_error.js.map