...
const TABLE_COLUMN = [`id`, `error_type`, `error_name`, `url_path`, `city_distribution_id`, `count_at_time`, `count_type`, `error_count`, `create_time`, `update_time`];
...
async function insertErrorSummaryRecord(projectId, countAt, countType, errorType, errorName, urlPath, cityDistributionId, errorCount) {
  ...
  const result = await Knex.returning('id').insert(insertData).into(tableName).catch(err => {
...
async function updateErrorSummaryRecord(id, projectId, countAt, countType, errorType, errorName, urlPath, errorCount) {
  ...
  const affecRows = await Knex(tableName).update(updateData).where('id', id).catch(err => {
    Logger.error(err.message);
    return 0;
  });
  return affecRows > 0;
...
async function replaceSummaryRecord(projectId, countAt, countType, errorType, errorName, urlPath, errorCount, cityDistrubutionJsonString) {
  ...
  if (_.isEmpty(rawRecord)) {
    // 如果不存在对应的记录
    // 先插城市分布数据获取id
    const cityDistributionId = await MCityDistribution.insertCityDistributionRecord(cityDistrubutionJsonString, projectId, countAt); // 再插errorSummary数据
    ...
    await MCityDistribution.updateCityDistributionRecord(cityDistributionId, projectId, countAt, cityDistrubutionJsonString); // 更新errorSummary记录
...
async function get(projectId, countAt, countType, errorType, errorName, urlPath) {
  ...
  const result = await Knex.select(TABLE_COLUMN).from(tableName).where(wherePrams).catch(err => {
    Logger.error(err.message);
    return [];
  });
...
async function getErrorNameDistributionListInSameMonth(projectId, startAt, endAt, countType, errorNameList = [], url = {}) {
  ...
  let rawRecordList = await Knex.select('error_name').sum('error_count as sum_error_count').from(tableName).where('count_type', countType).whereIn('count_at_time', countAtTimeList).whereIn('error_name', errorNameList).andWhere(extendCondition).groupBy('error_name').orderBy('sum_error_count', 'desc').catch(err => {
    Logger.error(err.message);
    return [];
  });
  let recordList = [];
...
async function getErrorNameList(projectId, errorType) {
  ...
  const rawRecordList = await Knex.select().distinct('error_name').from(tableName).where('count_type', DATE_FORMAT.UNIT.DAY).where('error_type', errorType).whereIn('count_at_time', timeList).catch(err => {
    Logger.error(err.message);
    return [];
  });
  const errorNameList = [];
...
async function getUrlPathDistributionListByErrorNameList(projectId, startAt, endAt, errorNameList, countType, max = 10) {
  const tableName = getTableName(projectId, startAt);
  let countAtTimeList = DatabaseUtil.getDatabaseTimeList(startAt, endAt, countType);
  let rawRecordList = await Knex.select('url_path').sum('error_count as total_count').from(tableName).where('count_type', countType).whereIn('error_name', errorNameList).whereIn('count_at_time', countAtTimeList).groupBy('url_path').orderBy('total_count', 'desc').limit(max).catch(err => {
    Logger.error(err.message);
    return [];
  });
  let recordList = [];
...
async function getStackAreaDistribution(projectId, startAt, endAt, countType, errorNameList = [], url = '') {
  ...
  let rawRecordList = await Knex.sum('error_count as sum_error_count').select(['error_name', 'count_at_time']).from(tableName).where('count_type', countType).andWhere(extendCondition).whereIn('count_at_time', timeList).whereIn('error_name', errorNameList).groupBy(['count_at_time', 'error_name']).catch(err => {
    Logger.error(err.message);
    return [];
  });
  let recordList = [];
...
async function getList(projectId, startAt, endAt, countType, errorNameList = [], url = '') {
  ...
  let rawRecordList = await Knex.select(TABLE_COLUMN).from(tableName).where('count_type', countType).whereIn('count_at_time', timeList).whereIn('error_name', errorNameList).andWhere(extendCondition).orderBy('error_count', 'desc').catch(err => {
    Logger.error(err.message);
    return [];
  });
...
async function getErrorNameDistributionInSameMonth(projectId, startAt, endAt, max = 500) {
  ...
  let rawDistributionList = await Knex.sum('error_count as sum_error_count').select('error_name').from(tableName).whereIn('count_at_time', timeList).andWhere('count_type', DATE_FORMAT.UNIT.DAY).groupBy('error_name').orderBy('sum_error_count', 'desc').limit(max).catch(e => {
...
async function getErrorSummaryByCountType(projectId, startAt, endAt, countType) {
  let tableName = getTableName(projectId, startAt);
  let timeList = DatabaseUtil.getDatabaseTimeList(startAt, endAt, countType);
  let rawResultList = await Knex.select(TABLE_COLUMN).from(tableName).where('count_type', countType).whereIn('count_at_time', timeList).catch(err => {
...
//# sourceMappingURL=error_summary.js.map