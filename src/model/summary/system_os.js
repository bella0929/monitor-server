...
const TABLE_COLUMN = [`id`, `project_id`, `os`, `os_version`, `total_count`, `count_at_month`, `city_distribute_id`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function summarySystemOs(visitAt) {
  ...
  for (let rawProject of projectList) {
    ...
    const sumRes = await Knex.count('* as total_count').select([`os`, `os_version`, `visit_at_month`, `country`, `province`, `city`]).from(systemTableName).where('visit_at_month', '=', visitAtMonth).groupBy('os').groupBy('os_version').groupBy('country').groupBy('province').groupBy('city').catch(err => {
      Logger.error(err);
      return [];
    });
...
async function replaceAndAutoIncreaseOsRecord(projectId, recordInfo, cityDistribute) {
  ...
  let oldRecordList = await Knex.select([`total_count`, `city_distribute_id`, `create_time`, `id`]).from(tableName).where('project_id', '=', projectId).andWhere('count_at_month', '=', countAtMonth).andWhere('os', '=', os).andWhere('os_version', '=', osVersion).catch(() => {
    return [];
...
//# sourceMappingURL=system_os.js.map