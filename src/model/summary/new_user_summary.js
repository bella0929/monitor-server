...
async function getNewUserDistribution(projectId, countType, timeList) {
  ...
  let rawRecordList = Knex.select(TABLE_COLUMN).from(tableName).where('project_id', projectId).andWhere('count_type', countType).whereIn('count_at_time', timeList).catch(err => {
...
//# sourceMappingURL=new_user_summary.js.map