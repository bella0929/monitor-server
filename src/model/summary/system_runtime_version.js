...
const TABLE_COLUMN = [`id`, `project_id`, `runtime_version`, `total_count`, `count_at_month`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function sumarySystemRuntimeVersion(visitAt) {
  ...
  for (let rawProject of projectList) {
    ...
    const sumRes = await Knex.count('* as total_count').select([`runtime_version`, `visit_at_month`, `country`, `province`, `city`]).from(systemTableName).where('visit_at_month', '=', visitAtMonth).groupBy('runtime_version').groupBy('country').groupBy('province').groupBy('city').catch(err => {
      Logger.error(err);
      return [];
    });
...
async function replaceAndAutoIncreaseRuntimeVersionRecord(projectId, recordInfo, cityDistribute) {
  ...
  let oldRecordList = await Knex.select([`total_count`, `city_distribute_id`, `create_time`, `id`]).from(tableName).where('project_id', '=', projectId).andWhere('count_at_month', '=', countAtMonth).andWhere('runtime_version', '=', runtimeVersion).catch(() => {
    return [];
...
//# sourceMappingURL=system_runtime_version.js.map