...
import Logger from '~/src/library/logger'; // 统计类别

const BASE_TABLE_NAME = 't_r_unique_view';
const TABLE_COLUMN = [`id`, `project_id`, `total_count`, `count_at_time`, `count_type`, `city_distribute_id`, `create_time`, `update_time`];
/**
 * 获取表名
...
async function replaceUvRecord(projectId, totalCount, countAtTime, countType, cityDistribute) {
  ...
  let oldRecordList = await Knex.select([`city_distribute_id`, `create_time`, `id`]).from(tableName).where('project_id', '=', projectId).andWhere('count_at_time', '=', countAtTime).andWhere('count_type', '=', countType).catch(() => {
    return [];
...
async function getRecord(projectId, countAtTime, countType) {
  let tableName = getTableName();
  let recordList = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', '=', projectId).andWhere('count_at_time', '=', countAtTime).andWhere('count_type', '=', countType).catch(e => {
    return [];
  });
...
async function getUVInRange(projectId, startAt, finishAt) {
  ...
  let rawRecord = await Knex.from(tableName).sum('total_count as total').where('count_type', '=', DATE_FORMAT.UNIT.HOUR).where('count_at_time', '>', startAtMoment).andWhere('count_at_time', '<', finishAtMoment).catch(e => {
    return 0;
  });
...
async function getRawRecordListInRange(projectId, startAt, endAt, countType) {
  ...
  let tableName = getTableName(projectId, startAt);
  let rawRecordList = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', projectId).andWhere('count_type', countType).whereIn('count_at_time', timeList).catch(err => {
...
//# sourceMappingURL=unique_view.js.map