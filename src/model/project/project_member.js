...
const TABLE_COLUMN = [`id`, `project_id`, `ucid`, `role`, `need_alarm`, `is_delete`, `create_time`, `create_ucid`, `update_time`, `update_ucid`];
const DISPLAY_TABLE_COLUMN = [`id`, `project_id`, `ucid`, `role`, `need_alarm`, `create_time`, `create_ucid`, `update_time`, `update_ucid`];
...
async function add(data) {
  ...
  for (let column of [`project_id`, `ucid`, `role`, `nead_alarm`, `create_ucid`, `update_ucid`]) {
...
async function get(id) {
  const tableName = getTableName();
  const result = await Knex.select(TABLE_COLUMN).from(tableName).where('id', '=', id).catch(err => {
...
async function getByProjectIdAndUcid(projectId, ucid) {
  ...
  let rawRecord = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', projectId).andWhere('ucid', ucid).catch(err => {
...
async function getList() {
  const tableName = getTableName();
  const result = await Knex.select(TABLE_COLUMN).from(tableName).where('is_delete', 0).catch(err => {
    Logger.log(err.message, 'project_member    getList   出错');
    return [];
  });
  return result;
...
async function update(id, updateData) {
  ...
  for (let column of ['project_id', 'ucid', 'role', 'need_alarm', 'is_delete', 'update_ucid']) {
...
async function getProjectMemberListByUcid(ucid, offset, max) {
  const tableName = getTableName();
  let rawRecordList = await Knex.select(TABLE_COLUMN).from(tableName).where('ucid', ucid).andWhere('is_delete', 0).offset(offset).limit(max).catch(err => {
...
async function getProjectMemberList(projectId) {
  const tableName = getTableName();
  const result = await Knex.select(TABLE_COLUMN).from(tableName).where('project_id', projectId).andWhere('is_delete', 0).catch(err => {
...
async function getAlarmUcidList(projectId) {
  const tableName = getTableName();
  const rawRecordList = await Knex.select('ucid').from(tableName).where('project_id', projectId).andWhere('need_alarm', 1).catch(() => {
    return [];
  });
  let result = [];
...
//# sourceMappingURL=project_member.js.map