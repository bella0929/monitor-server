...
const TABLE_COLUMN = [`id`, `ucid`, `account`, `email`, `password_md5`, `nickname`, `role`, `register_type`, `avatar_url`, `mobile`, `is_delete`, `create_time`, `update_time`];
/**
 * 对外展示的字段
 */

const DISPLAY_TABLE_COLUMN = [`ucid`, `account`, `email`, `nickname`, `role`, `register_type`, `avatar_url`, `mobile`];
/**
 * 加密字符串
 * @param {*} content
 */
...
async function register(account, userInfo) {
  const tableName = getTableName(); // 没有ucid,则把account转为ucid
  ...
  let avatarUrl = _.get(userInfo, ['avatarUrl'], DEFAULT_AVATAR_URL); // ucid和account不能为空
  ...
  let existUserByAccount = await getByAccount(account); // 检查account
  ...
  let insertResult = await Knex.returning('id').insert(insertData).into(tableName).catch(e => {
    return [];
  });
...
async function get(ucid) {
  ...
  const result = await Knex.select(TABLE_COLUMN).from(tableName).where('ucid', ucid);
...
async function getByAccount(account) {
  const tableName = getTableName();
  const result = await Knex.select(TABLE_COLUMN).from(tableName).where('account', account);
...
async function getSiteUserByAccount(account) {
  const tableName = getTableName();
  const result = await Knex.select(TABLE_COLUMN).from(tableName).where('account', account).andWhere('register_type', REGISTER_TYPE_SITE);
...
async function searchByAccount(account, offset = 0, max = 10) {
  const tableName = getTableName();
  const rescordList = await Knex.select(TABLE_COLUMN).from(tableName).where('is_delete', '=', 0).andWhere('account', 'like', `%${account}%`).limit(max).offset(offset);
  return rescordList;
}
/**
 * 用户列表
...
async function getList(offset = 0, max = 10) {
  const tableName = getTableName();
  const result = await Knex.select(TABLE_COLUMN).from(tableName).limit(max).offset(offset).where('is_delete', '=', 0);
  return result;
}
/**
 * 更新记录
...
async function update(ucid, rawUpdateData) {
  ...
  for (let key of ['email', 'password_md5', 'nickname', 'role', 'register_type', 'avatar_url', 'mobile', 'is_delete']) {
  ...
  const tableName = getTableName();
  const affectRows = await Knex(tableName).update(updateRecord).where('ucid', ucid);
  return affectRows > 0;
}
/**
...
async function getUserListByUcid(memberUcidList) {
  const tableName = getTableName();
  const result = await Knex.select(TABLE_COLUMN).from(tableName).whereIn('ucid', memberUcidList).catch(err => {
