...
const TABLE_COLUMN = [`id`, `project_name`, `display_name`, `rate`, `c_desc`, `create_time`, `create_ucid`, `update_time`, `update_ucid`, `is_delete`];
const DISPLAY_TABLE_COLUMN = [`id`, `project_name`, `display_name`, `rate`, `c_desc`, `create_time`, `create_ucid`, `update_time`, `update_ucid`];
...
async function add(data) {
  ...
  for (let column of [`project_name`, `display_name`, `c_desc`, `create_ucid`, `update_ucid`]) {
...
async function getList() {
  let tableName = getTableName();
  let result = await Knex.select(TABLE_COLUMN).from(tableName).where('is_delete', 0).catch(err => {
...
async function update(id, updateData) {
  ...
  for (let column of ['project_name', 'display_name', 'c_desc', 'is_delete', 'update_ucid']) {
