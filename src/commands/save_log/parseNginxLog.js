...
class NginxParseLog extends SaveLogBase {
  ...
  async execute(args, options) {
    ...
    let onDataIn = async (data, next) => {
      logCounter++;
      let content = data.toString(); // 获取日志时间, 没有原始日志时间则直接跳过
      ...
      legalLogCounter = legalLogCounter + 1; // 存原始数据
      ...
      jsonWriteStreamByLogCreateAt.write(JSON.stringify(parseResult)); // 定期清一下
...
//# sourceMappingURL=parseNginxLog.js.map