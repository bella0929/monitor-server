...
class SaveLogBase extends Base {
  ...
  */
  ...
  getWriteStreamClientByType(nowAt, logType = Nginx.LOG_TYPE_RAW) {
    ...
    this.log(`清洗后的日志路径 `, nowAtLogUri); // 创建对应路径
    ...
    if (jsonWriteStreamPool.has(nowAtLogUri)) {
      ...
      nowAtWriteStream = writeLine(fs.createWriteStream(nowAtLogUri, {
        flags: 'a'
      }), {
        // 换行符，默认\n
        newline: '\n',
  ...
  autoCloseOldStream(isCloseAll = false) {
    ...
    let needCloseLogUriSet = new Set(); // 获得所有过期uri key
  ...
  parseLogCreateAt(data) {
    ...
    let url = _.get(info, [15], ''); //this.log('parseLogCreateAt url ', url);
    ...
    let record = _.get(urlQS, ['query', 'd'], '{}'); //this.log('parseLogCreateAt record ', record);
    ...
    let logAtMoment = moment(info[0], moment.ISO_8601); //this.log('logAtMoment', logAtMoment);
  ...
  async parseLog(data, projectMap) {
    ...
    this.log('parseNginxLog record ', record); // 记录日志md5
    ...
    let logCreateAt = this.parseLogCreateAt(data); // 如果入库时间距离现在大于10天, 则认为是不合法数据(kafka中只会存7天以内的数据, 入库时间超出上下10天, 都不正常)
    ...
    record.time = logCreateAt; // 新版中info[17] 里有%号, 是非法字符, 需要提前处理

    let safeInfo16 = _.replace(info[16], '%', ''); //this.log('info[16]',info[16]);


    this.log('ua', safeInfo16);
    record.ua = parser(decodeURIComponent(safeInfo16));
    this.log('record.ua', record.ua); // 兼容处理saas系统打点UA问题, nwjs低版本下获取不到chrome的版本, 解析拿到的为chromium_ver
    ...
    if (record.ip === '127.0.0.1') record.ip = '59.109.146.215'; //this.log('info[3]',info[3]);
