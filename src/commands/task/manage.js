...
class TaskManager extends Base {
  ...
  async closeOtherTaskManager() {
    ...
    Alert.sendMessage(WatchIdList.WATCH_UCID_LIST_DEFAULT, alertMessage); // 花式自尽
  ...
  async registerTaskRepeatPer10Minute() {
    ...
    schedule.scheduleJob('15 */10 * * * * *', function () {
      ...
      let fifteenMinuteAgoByminute = moment().subtract(15, DATE_FORMAT.UNIT.MINUTE).format(DATE_FORMAT.COMMAND_ARGUMENT_BY_MINUTE); // 周期性执行命令
      ...
      let parseCommandList = ['Parse:UV', 'Parse:TimeOnSiteByHour', 'Parse:Performance', 'Parse:Monitor'];
      ...
      let summaryCommandList = ['Summary:UV', 'Summary:NewUser', 'Summary:Performance', 'Summary:Error'];

      for (let summaryCommand of summaryCommandList) {
        // 当前小时
        that.dispatchParseCommand(summaryCommand, nowByHour, DATE_FORMAT.UNIT.HOUR); // 一小时前
  ...
  async registerTaskRepeatPer1Hour() {
    ...
    schedule.scheduleJob('30 15 * * * * *', function () {
      ...
      let nowByMinute = moment().format(DATE_FORMAT.COMMAND_ARGUMENT_BY_MINUTE);
      let lastDayStartAtByMinute = moment().subtract(1, DATE_FORMAT.UNIT.DAY).startOf(DATE_FORMAT.UNIT.DAY).format(DATE_FORMAT.COMMAND_ARGUMENT_BY_MINUTE); // 解析命令

      let parseCommandList = ['Parse:Device', 'Parse:MenuClick', 'Parse:UserFirstLoginAt'];
      ...
      let summaryCommandList = ['Summary:UV', 'Summary:NewUser', 'Summary:Performance', 'Summary:Error', 'Summary:TimeOnSite'];
  ...
  async registerTaskRepeatPer6Hour() {
    ...
    schedule.scheduleJob('45 35 */6 * * * *', function () {
      ...
      let oneDayAgoByDay = oneDayAgo.format(DATE_FORMAT.COMMAND_ARGUMENT_BY_DAY); // 汇总命令-昨日数据
      ...
      let summaryByMonthCommandList = ['Summary:UV', 'Summary:NewUser', 'Summary:Performance', 'Summary:TimeOnSite', 'Summary:SystemBrowser', 'Summary:SystemDevice', 'Summary:SystemOS' // 'Summary:SystemRuntimeVersion'
      ];

      for (let summaryCommand of summaryByMonthCommandList) {
        // 当月数据
        that.dispatchParseCommand(summaryCommand, nowByMonth, DATE_FORMAT.UNIT.MONTH); // 上月数据
  ...
  async dispatchParseCommand(commandName, startAtStr, endAtStr) {
    ...
    this.execCommand(commandName, [startAtStr, // startAtYmdHi:
    endAtStr // endAtYmdHi
    ]);
  }
  /**
  ...
  async dispatchSummaryCommand(commandName, countAtStr, summaryType) {
    ...
    this.execCommand(commandName, [countAtStr, // summaryAtTime:
