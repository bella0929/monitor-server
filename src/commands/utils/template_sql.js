...
const SQL_DATE_FORMAT_YM = 'YYYYMM'; // 不需要分表
...
function generate(baseTableName, projectId = '', tableTime = '') {
  // 获取模板
  let content = TABLE_TEMPLATE[baseTableName]; // 生成表名
...
class GenerateSQL extends Base {
  ...
  async execute(args, options) {
    ...
    let finishAtYm = moment().format('YYYYMM'); //     let commonSqlContent = `
    // SET NAMES utf8mb4;
    ...
    for (let tableName of [SINGLE_T_O_PROJECT, SINGLE_T_R_BEHAVIOR_DISTRIBUTION, SINGLE_T_R_DURATION_DISTRIBUTION, SINGLE_T_R_HTTP_ERROR_DISTRIBUTION, SINGLE_T_R_PAGE_VIEW, SINGLE_T_R_SYSTEM_BROWSER, SINGLE_T_R_SYSTEM_RUNTIME_VERSION, SINGLE_T_R_SYSTEM_DEVICE, SINGLE_T_R_SYSTEM_OS, SINGLE_T_R_UNIQUE_VIEW, SINGLE_T_O_ALARM_CONFIG, SINGLE_T_O_USER, SINGLE_T_O_PROJECT_MEMBER, SINGLE_T_O_NEW_USER_SUMMARY, SINGLE_T_R_ALARM_LOG]) {
    ...
    for (let projectId of projectIdList) {
      for (let currentAtMoment = moment(startAtYm, DATE_FORMAT.COMMAND_ARGUMENT_BY_MONTH); // 生成时间范围之内的数据表
      currentAtMoment.isBefore(finishAtMoment.clone().add(1, 'months')); currentAtMoment = currentAtMoment.clone().add(1, 'months')) {
        ...
        for (let tableName of [MUILT_T_O_MONITOR, MUILT_T_O_MONITOR_EXT, MUILT_T_O_UV_RECORD, MUILT_T_R_CITY_DISTRIBUTION, MUILT_T_R_PERFORMANCE, MUILT_T_O_SYSTEM_COLLECTION, MUILT_T_O_USER_FIRST_LOGIN_AT, MUILT_T_R_ERROR_SUMMARY]) {
...
//# sourceMappingURL=template_sql.js.map