...
class CommandReProcessLog extends Base {
  ...
  async runParse(startAtYmdHi, endAtYmdHi) {
    ...
    await this.execCommand('Parse:Monitor', [startAtYmdHi, // startAtYmdHi:
    ...
    await this.execCommand('Parse:UV', [startAtYmdHi, // startAtYmdHi:
    endAtYmdHi // endAtYmdHi:
  ...
  async runSummaryByHour(startAtYmdHi, endAtYmdHi) {
    ...
    for (let processAt = startAt; processAt <= endAt; processAt += 3600) {
      ...
      await this.execCommand('Summary:UV', [formatHour, // countAtTime:
  ...
  async runSummaryByDay(startAtYmdHi, endAtYmdHi) {
    ...
    for (let processAt = startAt; processAt <= endAt; processAt += 3600 * 24) {
      ...
      await this.execCommand('Summary:UV', [formatDay, // countAtTime:
      ...
      await this.execCommand('Summary:HttpError', [formatDay, // sumaryAtTime:
      DATE_FORMAT.UNIT.DAY // countType:
      ]); // [按天/按月] 根据历史数据, 汇总分析记录指定时间范围内用户停留时长

      this.log(`[按天] Summary:TimeOnSite任务开始, 处理时间 => ${formatDay}`);
      await this.execCommand('Summary:TimeOnSite', [formatDay, // countAtTime:
      DATE_FORMAT.UNIT.DAY // countType:
      ]);
    }
  }
  ...
  async runSummaryByMonth(startAtYmdHi, endAtYmdHi) {
    ...
    for (let processMoment = startMonth.clone(); processMoment.unix() <= endMonthAt; processMoment = processMoment.clone().add(1, 'month')) {
      ...
      await this.execCommand('Summary:SystemBrowser', [formatMonth, // sumaryYm:
      DATE_FORMAT.UNIT.MONTH]); // t_r_system_device 设备信息统计 summary 按月统计 每天跑

      this.log(`[按天] Summary:SystemDevice任务开始, 处理时间 => ${formatMonth}`);
      await this.execCommand('Summary:SystemDevice', [formatMonth, // sumaryYm:
      DATE_FORMAT.UNIT.MONTH]); // t_r_system_os 操作系统统计 按月统计 每天跑

      this.log(`[按天] Summary:SystemOS任务开始, 处理时间 => ${formatMonth}`);
      await this.execCommand('Summary:SystemOS', [formatMonth, // sumaryYm:
      DATE_FORMAT.UNIT.MONTH]); // t_r_project_version 按项目版本进行统计 按月统计 每天跑

      this.log(`[按天] Summary:SystemRuntimeVersion任务开始, 处理时间 => ${formatMonth}`);
      await this.execCommand('Summary:SystemRuntimeVersion', [formatMonth, // sumaryYm:
      DATE_FORMAT.UNIT.MONTH]); // [按月] 根据历史数据, 汇总分析记录指定时间范围内的uv

      this.log(`[按月] Summary:UV任务开始, 处理时间 => ${formatMonth}`);
      await this.execCommand('Summary:UV', [formatMonth, // countAtTime
      DATE_FORMAT.UNIT.MONTH // countType
      ]); // [按月] 基于数据表做统计, 按月统计browser占比

      this.log(`[按月] Summary:HttpError任务开始, 处理时间 => ${formatMonth}`);
      await this.execCommand('Summary:HttpError', [formatMonth, // sumaryAtTime:
      DATE_FORMAT.UNIT.MONTH // countType:
      ]); // [按月] 根据历史数据, 汇总分析记录指定时间范围内用户停留时长

      this.log(`[按月] Summary:TimeOnSite任务开始, 处理时间 => ${formatMonth}`);
      await this.execCommand('Summary:TimeOnSite', [formatMonth, // countAtTime
      DATE_FORMAT.UNIT.MONTH // countType
      ]);
    }
  }
...
//# sourceMappingURL=re_process_log.js.map