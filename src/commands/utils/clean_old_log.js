...
class CleanOldLog extends Base {
  ...
  async clearOldKafkaLog() {
    ...
    let oneMonthAgoAt = currentAtMoment.clone().subtract(1, DATE_FORMAT.UNIT.MONTH).unix(); // 每月20号后删除上一个月的数据
  ...
  async clearOldCommandLog() {
    ...
    let currentAtMoment = moment(); // 每月20号后删除上一个月的数据
...
//# sourceMappingURL=clean_old_log.js.map