...
class UVSummary extends Base {
  ...
  async execute(args, options) {
    ...
    for (let rawProject of rawProjectList) {
      ...
      this.log(`[${projectId}(${projectDesc})] 时间范围:${startAtMoment.format(DATE_FORMAT.DISPLAY_BY_MINUTE) + ':00'}~${endAtMoment.format(DATE_FORMAT.DISPLAY_BY_MINUTE) + ':59'}`); // 如果是hour，则用原始表数据
  ...
  async handleOther(projectId, startAt, endAt, countAtMoment, countType, projectDesc) {
    ...
    MUniqueView.replaceUvRecord(projectId, totalUv, countAtMoment.format(DATE_FORMAT.DATABASE_BY_UNIT[countType]), countType, cityDistribute);
  }
