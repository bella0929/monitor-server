...
class SummaryError extends Base {
  ...
  async execute(args, options) {
    ...
    for (let rawProject of rawProjectList) {
      ...
      this.log(`[${projectId}(${projectName})] 时间范围:${startAtMoment.format(DATE_FORMAT.DIAPLAY_BY_MINUTE) + ':00'}~${endAtMoment.format(DATE_FORMAT.DIAPLAY_BY_MINUTE) + ':59'}`); // 如果countType是分钟，查询原始数据表
  ...
  async handleOther(projectId, projectName, startAt, endAt, countType) {
    ...
    for (let errorName of errorNameList) {
      ...
      for (let url of urlList) {
        ...
        let idList = _.get(resultMap, [errorName, url, 'idList'], []); // 开始处理城市分布数据
        // 先处理城市分布数据
      ...
      for (let errorName of Object.keys(errorTypeNameUrlMap[errorType])) {
        for (let urlPath of Object.keys(errorTypeNameUrlMap[errorType][errorName])) {
          const {
            cityDistribution,
            errorCount
          } = _.get(errorTypeNameUrlMap, [errorType, errorName, urlPath], {}); // 替换errorSummary数据
