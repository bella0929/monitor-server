...
class ParseMonitor extends ParseBase {
  ...
  isLegalRecord(record) {
    ...
    if ((errorTypeStr === MMonitor.ERROR_TYPE_页面加载异常 || errorTypeStr === MMonitor.ERROR_TYPE_启动异常 || errorTypeStr === MMonitor.ERROR_TYPE_登录异常 || errorTypeStr === MMonitor.ERROR_TYPE_NODE报错 || errorTypeStr === MMonitor.ERROR_TYPE_JS异常 || errorTypeStr === MMonitor.ERROR_TYPE_自定义异常) && !errorName) {
      return false;
    }
    ...
    for (let [projectId, visitAtMap] of this.projectMap) {
      for (let [visitAtTime, monitorMap] of visitAtMap) {
        ...
        for (let monitorRecord of monitorMap) {
          ...
          const sqlRecord = Util.handleEmptyData(sqlParams); // monitor查询参数
          ...
          this.log('===monitorParams===', monitorParams); // monitor_ext查询更新参数
