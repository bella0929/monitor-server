...
class ParseBase extends Base {
  constructor() {
    super(); // 初始化属性(目前只能在constructor里注册属性)
    // 统一按项目进行统计
  ...
  isLegalRecord(record) {
    this.mustBeOverride(); // let recordType = get(record, ['type'], '')
  ...
  async processRecordAndCacheInProjectMap(record) {
    this.mustBeOverride(); // let projectId = get(record, ['project_id'], 0)
  ...
  async save2DB() {
    ...
    let totalRecordCount = this.getRecordCountInProjectMap(); // 处理的时候调一下这个方法, 专业打印处理进度

    this.reportProcess(processRecordCount, successSaveCount, totalRecordCount); // for (let [projectId, countAtMap] of projectMap) {
  ...
  getRecordCountInProjectMap() {
    this.mustBeOverride();
    let totalCount = 0; // for (let [projectId, countAtMap] of projectMap) {
    //   for (let [countAtTime, distribution] of countAtMap) {
