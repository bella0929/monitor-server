...
class ParseUV extends ParseBase {
  ...
  async processRecordAndCacheInProjectMap(record) {
    ...
    let city = _.get(record, ['city'], ''); //this.log('uv record',record);
    ...
    uvMap.set(uuid, uvRecord); //this.log('uvRecord',uvRecord);
      ...
      for (let [visitAtHour, uvMap] of visitAtMap) {
        ...
        for (let [key, uvRecord] of uvMap) {
          ...
          if (existUuidSet.has(uuid) === false) {
            // 只有当uuid不存在时才插入
            isSuccess = await MUvRecord.replaceUvRecord(projectId, uuid, visitAt, country, province, city); // 插入完成后, 把uuid再补进去(意义不大, 只为了增强稳定性)
