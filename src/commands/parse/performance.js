...
class ParseUV extends ParseBase {
  ...
  replaceIndicatorRecord(indicator, indicatorValue, projectId, url, countAtMinute, country, province, city) {
    ...
    this.log('performance locationPath ', locationPath); // 初始化Map对象
    ...
    uniqIdMap.set(locationPath, record); // 检查projectMap中是否有对应记录, 如果有, 使用projectMap中的记录
    ...
    record['pv'] += 1;
    this.log('performance record ', record); // 将记录更新回Map中
  ...
  async processRecordAndCacheInProjectMap(record) {
    ...
    let city = _.get(record, ['city'], ''); // 数据都在detail里
    ...
    let visitAtMinute = moment.unix(visitAt).format(DATE_FORMAT.DATABASE_BY_MINUTE); // 计算响应指标数据

    for (let indicator of [//  资源加载指标
    MPerformance.INDICATOR_TYPE_DNS查询耗时, MPerformance.INDICATOR_TYPE_TCP链接耗时, MPerformance.INDICATOR_TYPE_请求响应耗时, MPerformance.INDICATOR_TYPE_DOM解析耗时, MPerformance.INDICATOR_TYPE_内容传输耗时, MPerformance.INDICATOR_TYPE_资源加载耗时, MPerformance.INDICATOR_TYPE_SSL连接耗时, // 关键性能指标
    MPerformance.INDICATOR_TYPE_首包时间耗时, MPerformance.INDICATOR_TYPE_首次渲染耗时, MPerformance.INDICATOR_TYPE_首次可交互耗时, MPerformance.INDICATOR_TYPE_DOM_READY_耗时, MPerformance.INDICATOR_TYPE_页面完全加载耗时]) {
